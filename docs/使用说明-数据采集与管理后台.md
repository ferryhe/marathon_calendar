...
2. Sources：查看/启用/调整 priority/编辑 config(JSON)
3. Sync：点击“立即同步一次”触发一次同步
4. Raw Crawl：默认查看 `needs_review`，点击“复核/回填”进入处理流程
5. Discovery：用 Brave 搜索找到候选页面，再做绑定（Bind Marathon Source）
