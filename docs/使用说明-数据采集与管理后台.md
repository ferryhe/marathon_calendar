...
补充阅读（赛事审核专项）：`docs/使用说明-赛事审核与规则模板.md`
...
- AI 规则模板生成（默认关闭）：`AI_API_KEY`、`AI_MODEL`、`AI_ENABLE_RULE_GEN=true`
...
### 平台列表发现（最酷/爱燃烧）
如果平台有“赛事列表页”，可以用 `/admin/data` 的 “平台列表发现（HTML）” 批量发现详情页链接，再用 “Bind Marathon Source” 绑定到具体赛事与 Source。

前提：
- 对应 Source 的 `sources.config.discovery.list.itemLink.selector` 已配置（可在 Sources Tab 编辑）
- 示例：
  - 最酷体育列表页：`https://www.zuicool.com/events`
  - 爱燃烧（内容站）可从分类页发现：`https://iranshao.com/category/race`
