...
6. 提取：优先 JSON-LD Event（`application/ld+json`）提取 `startDate` 作为 `raceDate`
   - 若 JSON-LD 不存在/不完整，支持 `sources.config.extract` 的配置化提取（selector/attr/regex）
...
3. 核心官网真实验证（2026-02-10）：
   - `npx tsx script/verify-core-official-crawl.ts`
   - 覆盖 5 个知名赛事官网页面：Tokyo / Paris / Boston / Berlin / Chicago
   - 产物：写入 `raw_crawl_data`、记录 `marathon_sync_runs`，并更新 `marathon_editions`（以提取到的 raceDate 为准）
...
- 详细计划：`docs/项目计划-阶段一-1.3-数据采集与调度-详细计划.md`
 - 核心官网验证脚本：`script/verify-core-official-crawl.ts`
