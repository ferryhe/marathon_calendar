...
| 日期 | 脚本 | 影响 | 状态 |
|---|---|---|---|
| 2026-05-02 | [第三方平台绑定与赛事数据修正](./2026-05-02-第三方平台绑定与赛事数据修正.sql) | sources +5 / marathon_sources +13 / marathon_editions update 11（含开普敦 10→5 月修正） | ⏳ 待同步至 prod |
| 2026-05-02c | [生产库初始化（dev 全量导出）](./2026-05-02c-prod-initial-seed.sql) | sources 20 / marathons 37 / marathon_editions 41 / marathon_sources 45（**全部 ON CONFLICT 幂等**，已包含上一条增量内容） | ⏳ 待对 prod 执行（一次即可） |

> 说明：`2026-05-02c-prod-initial-seed.sql` 是 dev 库当前**完整快照**，已经把上一条增量包在里面。生产库目前为空，**只需对 prod 跑 c 这一份即可**。后续做小修小改时，再用单独的增量脚本。
